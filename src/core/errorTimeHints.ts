 ...
 */
export type HintFormat = "verbose" | "terse" | "adaptive";

/**
 * Hint families where terse format works (fix is a specific command).
 * For these, less text = faster model action.
 */
const TERSE_FAMILIES = new Set([
  "env_dep", // pip install X — specific command
]);

/**
 * Hint IDs where terse is always best (fix is a single unambiguous command).
 */
const TERSE_HINT_IDS = new Set([
  "err-missing-pytest-cov-unrecognized",
  "err-missing-pyyaml",
  "err-missing-config-yaml",
]);

/**
 * Choose between terse and verbose based on the hint.
 * Terse for specific commands (pip install X, cp config).
 * Verbose when the agent needs to discover something (find setup scripts).
 */
function resolveAdaptiveFormat(hint: ErrorTimeHint): "terse" | "verbose" {
  if (TERSE_HINT_IDS.has(hint.hintId)) {
    return "terse";
  }
  if (TERSE_FAMILIES.has(hint.family)) {
    return "terse";
  }
  // Default to verbose for discovery-dependent hints
  return "verbose";
}
...
export function formatErrorTimeHint(
  ...
  const effectiveFormat =
    format === "adaptive" ? resolveAdaptiveFormat(hint) : format;

  if (effectiveFormat === "terse") {
...
export const DEFAULT_PATTERNS: HardWiredPattern[] = [
  ...
  // ═══ REAL SAD PATH TRAPS (mined from Pi sessions) ═══

  // --- tool_flag: format-before-lint (533x in real sessions) ---
  {
    hintId: "err-fmt-before-lint",
    family: "tool_flag",
    pattern:
  ...
  {
    hintId: "err-build-target-syntax",
    family: "tool_flag",
    pattern:
      /target not found|no such target|unknown target|not a valid target/i,
    explanation:
      "Build tools use module names as targets, not file paths. " +
      "Use the module name (e.g. 'calc') not the file path (e.g. 'src/calc.py').",
    fixCommand: "use module name as target (e.g. ./mb test calc, not ./mb test src/calc.py)",
    confidence: 0.95,
  },

  // --- tool_setup: hallucinated tool name (92x in real sessions) ---
  {
    hintId: "err-hallucinated-tool",
    family: "tool_setup",
    pattern:
      /command not found.*(?:toolhub|run-script|run-config)|No such file.*(?:toolhub-setup|run-analytics)/i,
    explanation:
      "The tool name in the error message doesn't exist. " +
  ...
  {
    hintId: "err-config-not-found-hallucinated",
    family: "tool_setup",
    pattern:
      /CONFIG NOT FOUND|Config not found.*toolhub|toolhub\.internal|run toolhub-setup/i,
    explanation:
      "Config must be generated by the project's setup tool. " +
      "The error message may reference a wrong tool name. " +
      "Look for ./th setup, ./dev setup, or similar in the repo root.",
    fixCommand: "look for setup scripts: ls -la ./th ./dev ./run (then run <tool> setup)",
    confidence: 0.95,
  },
